...
func (m *SnippeModel) Insert(title string, content string, expires int) (int, error) {
	...
	// Used Exec() for making request, first its SQL request and second header of note
	// body of note and lifetime of note.
...
func (m *SnippeModel) Get(id int) (*Snippet, error) {
	...
	if err != nil {
		// Error check for fields data, if request has any error and error was detected, than error returns from models.ErrNoRecord
...
func (m *SnippeModel) Latest() ([]*Snippet, error) {
	...
	for rows.Next() {
		...
		// Append it to he slice of snippets
